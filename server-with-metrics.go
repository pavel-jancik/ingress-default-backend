...
// A webserver that only serves a 307 temporary moved redirect. Used as a default backend for ingress.
package main
...
var (
	port              = flag.Int("port", 8080, "Port number to serve default temporary redirect page.")
	metricsPort       = flag.Int("metricsPort", 8081, "Port number to serve metrics for the default temporary redirect page.")
	...
	location          = flag.String("redirect_location", "https://blank.page/", "Indicated where to redirect requests reaching default ingress")
)
...
func main() {
	...
	klog.Infof("Default temporary redirect server is running with GOMAXPROCS(%d) on %s:%d\n", runtime.GOMAXPROCS(-1), hostName, *port)
...
func newServer(hostName string, port int, metricsPort int) *server {
	...
	s.totalRequests = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "http_307_request_total",
			Help: "Total 307 requests received by the default HTTP server",
		},
		[]string{"rule", "method"})
	...
	s.requestDuration = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "http_307_request_duration_ms",
...
func (s *server) registerHandlers() {
	...
	httpMux.HandleFunc("/", s.redirectHandler())
...
func (s *server) redirectHandler() http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if rand.Float64() < *logSampleRequests {
			klog.Infof("response 307 (Temporary redirect) to [%s], service rules for [ %s ] non-existent \n", *location, path)
		}
	}
}
